...
% linear regression
regularization = 'ridge';    % :: default: lasso, check docs for available options
solver = 'lbfgs';            % :: default: sparsa, check docs for available options
...
base_path = 'W:/home/marco/projects/FYP/result/Standard'; % please change to your workspace


% can modify file name based on your params
fname1 = sprintf('%s/%s-model_%d-iter_%.1f-dist_%.1f-corr.png', base_path, model, n_iteration, distance_cutoff, correlation_cutoff);
fname1_chain1 = sprintf('%s/%s-model_%d-iter_%.1f-dist_%.1f-corr_1-chain.png', base_path, model, n_iteration, distance_cutoff, correlation_cutoff);
fname1_chain2 = sprintf('%s/%s-model_%d-iter_%.1f-dist_%.1f-corr_2-chain.png', base_path, model, n_iteration, distance_cutoff, correlation_cutoff);
fname2 = sprintf('%s/%s-model_%d-iter_%.1f-dist_%.1f-corr.csv', base_path, model, n_iteration, distance_cutoff, correlation_cutoff);
...
ftitle = sprintf('%s %d-iter %.1f-dist %.1f-corr 1-chain',model,  n_iteration, distance_cutoff, correlation_cutoff);
title(ftitle, FontSize = 16)
xlabel('residue', FontSize = 13)
ylabel('importance', FontSize = 13)
...
ftitle = sprintf('%s %d-iter %.1f-dist %.1f-corr 2-chain',model,  n_iteration, distance_cutoff, correlation_cutoff);
title(ftitle, FontSize = 16)
xlabel('residue', FontSize = 13)
ylabel('importance', FontSize = 13)
